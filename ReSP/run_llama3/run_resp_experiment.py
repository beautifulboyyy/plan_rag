...
# 添加 ReSP 目录到路径（run_llama3 的上一级）
resp_dir = os.path.dirname(os.path.abspath(__file__))
resp_dir = os.path.dirname(resp_dir)  # ReSP 目录
...
def run_resp_experiment(args):
    """运行本地HF模型 ReSP 实验"""

    # 设置保存标识
    save_note = f"resp_llama3_hf_{args.split}"
    ...
    # 加载配置
    config_file_path = os.path.join(os.path.dirname(__file__), "resp_hf_config.yaml")
    config = Config(config_file_path=config_file_path, config_dict=config_dict)

    # 修正路径为绝对路径（项目根目录 planrag）
    # ReSP/run_llama3 -> ReSP -> planrag (项目根目录)
    resp_root = os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))

    if 'data_dir' in config and not os.path.isabs(config['data_dir']):
        config['data_dir'] = os.path.normpath(os.path.join(resp_root, config['data_dir']))
    ...
    if 'index_path' in config and not os.path.isabs(config['index_path']):
        config['index_path'] = os.path.normpath(os.path.join(resp_root, config['index_path']))
        print(f"索引路径: {config['index_path']}")

    if 'corpus_path' in config and not os.path.isabs(config['corpus_path']):
        config['corpus_path'] = os.path.normpath(os.path.join(resp_root, config['corpus_path']))
        print(f"语料库路径: {config['corpus_path']}")

    if 'retrieval_model_path' in config and not os.path.isabs(config['retrieval_model_path']):
        config['retrieval_model_path'] = os.path.normpath(os.path.join(resp_root, config['retrieval_model_path']))
    ...
    if 'generator_model_path' in config and not os.path.isabs(config['generator_model_path']):
        config['generator_model_path'] = os.path.normpath(os.path.join(resp_root, config['generator_model_path']))
        print(f"生成器模型路径: {config['generator_model_path']}")

    if 'save_dir' in config and not os.path.isabs(config['save_dir']):
        config['save_dir'] = os.path.normpath(os.path.join(resp_root, config['save_dir']))
    ...
    # 创建 ReSP Pipeline
    print("正在初始化 ReSP Pipeline (使用本地 HF Llama 3)...")
    pipeline = ReSPPipeline(config)
...
def main():
    parser = argparse.ArgumentParser(description="运行本地HF模型 ReSP 实验")
